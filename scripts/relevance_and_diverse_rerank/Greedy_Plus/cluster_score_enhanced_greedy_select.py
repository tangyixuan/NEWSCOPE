...
parser.add_argument("--w_cluster", type=int, help="w_cluster*cluster_score + w_sim*similarity_score")
parser.add_argument("--w_sim", type=int, help="w_cluster*cluster_score + w_sim*similarity_score")
parser.add_argument("--news_text_dir", type=str, default="news_text_by_paragraph")
parser.add_argument("--greedy_select_result_dir", type=str, default="cluster_score_based_greedy_select")
parser.add_argument("--relevance_score_dir", type=str, default="reranker_score")
parser.add_argument("--sentences_cluster_dir", type=str, default="HDBSCAN_sent_cluster")
...
greedy_select_result_path = os.path.join(data_root_path, f"{args.greedy_select_result_dir}_cluster_{str(w_cluster)}_sim_{str(w_sim)}")
...
for news_file_name in os.listdir(relevance_score_path):
    ...
    if os.path.exists(os.path.join(sentences_cluster_result, news_file_name)):
        ...
        for cluster_id in cluster_result_dict:
            for d in cluster_result_dict[cluster_id]:
                ...
                # sentences.append(unicodedata.normalize('NFKC', d["text"].lower()))
    ...
    #     pass
        ...
        for para_id, origin_para in enumerate(paragraphs):
            ...
            for sent in split_para_sentences:
                if sent.strip():
                    sent_id = len(sentences)
                    # sentences.append(unidecode(sent.lower()).replace("'", "").replace("\"", ""))
        ...
        sentence_representations = sentence_embeddings
        ...
        ######################cluster by OPTICS######################
        # clusterer = OPTICS(metric='precomputed', min_samples = 2, xi = 0.05, min_cluster_size = 2)
        clusterer = OPTICS(metric='precomputed', min_samples = 2)
        # clusterer = hdbscan.HDBSCAN(metric='precomputed', min_samples = 1, gen_min_span_tree=True, cluster_selection_method='leaf', min_cluster_size = 2, cluster_selection_epsilon=0.03)
    ...
    cluster_scores = {}
    for cluster_id in cluster_to_sentences:
        cluster_score = np.mean([relevance_score[idto_id[sent_to_para_mapping[sent_id]]] for sent_id in cluster_to_sentences[cluster_id]])
    ...
    while cnt < max_top_k:
        ...
        for para_id in range(len(paragraphs)):
            ...
            # diversity_score = sum([cluster_scores[cluster_id] for cluster_id in new_clusters if cluster_id != -1])
            diversity_score = sum([cluster_scores[cluster_id] for cluster_id in new_clusters])
